...
machin_power <- function(s1, s2, e2, k = 1, mu = 0, sig.level = .05, alternative = c("two.sided", "less", "greater")) {
  alternative <- match.arg(alternative)

  e1 <- k * e2
  z <- (s2 - s1 - mu) / sqrt((s1 * (1 - s1) / e1) + (s2 * (1 - s2) / e2))

  if (alternative == "two.sided") {
    pnorm(z - qnorm(1 - sig.level / 2)) + pnorm(-z - qnorm(1 - sig.level / 2))
  } else if (alternative == "greater") {
    pnorm(z - qnorm(1 - sig.level))
